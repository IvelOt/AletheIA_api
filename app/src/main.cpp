...
int main(int argc, char** argv) {
    ...
    if (!config.load(".env") && !config.load("../../.env")) {
        std::cerr << "[Aletheia Edge] Aviso: .env não encontrado." << std::endl;
    }
    ...
    try {
        engine = std::make_unique<AletheiaEngine>(classifier_path, isf_model_path);
    } catch (const std::exception& e) {
        std::cerr << "[Aletheia Edge] ERRO FATAL: " << e.what() << std::endl;
        return 1;
    }

    httplib::Server svr;

    // Rota Única: Verificação Stateless (Selfie vs Documento)
    ...
    std::cout << "[Aletheia Edge] API Stateless pronta em http://" << bind_addr << ":" << port << std::endl;
